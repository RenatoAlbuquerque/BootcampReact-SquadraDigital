...
class BairroDao extends AbstractDao
{
    public async incluirBairro(bairroVo : BairroVo)
    {
        ...
        {
            throw new IncluirError("bairro", "Erro ao inserir no banco de dados", 404, error);
        }
    }
    ...
    public async alterarBairro(bairroVo : BairroVo)
    {
        ...
        try
        {
            ...
            console.log('QUANTIDADE DE REGISTROS ALTERADOS (BAIRROS): ' + resultado.rowsAffected);
            if(resultado.rowsAffected == 0)
            {
                throw new AlterarError("bairro", "Não existe bairro com o código " + bairroVo.codigoBairro, 404, null);
            }
        }
        catch(error)
        {
            if(error instanceof AlterarError) {throw error} else {throw new AlterarError("bairro", "Erro ao alterar no banco de dados", 404, error);}
        }
    }
    ...
    public async pesquisarBairro(bairroVoFiltroPesquisa : BairroVo) : Promise<any>
    {
        try
        {
            let recursos : any[] = this.gerarSQLConsultarListar(bairroVoFiltroPesquisa);
            let sql = recursos[0]; //sql
            let parametros : any[] = recursos[1]; //parametros
            let resultSet = await this.conexao.execute(sql, parametros);
            console.log('QUANTIDADE DE REGISTROS ENCONTRADOS (BAIRROS): ' + resultSet.rows.length);
            let retorno = (resultSet.rows.length == 1 ? this.buscarUmRegistro(resultSet) : this.buscarVariosRegistros(resultSet));
            return retorno;
        }
        catch(error)
        {
            throw new ConsultarError("bairro", "Erro ao pesquisar no banco de dados", 404, error);
        }
    }
