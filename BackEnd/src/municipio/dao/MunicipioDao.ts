...
class MunicipioDao extends AbstractDao
{
    ...
    public async alterarMunicipio(municipioVo : MunicipioVo)
    {
        ...
        {
            if(error instanceof AlterarError) {throw error} else {throw new AlterarError("município", "Erro ao alterar no banco de dados", 404, error);}
        }
    }
    ...
    public async pesquisarMunicipio(municipioVoFiltroPesquisa : MunicipioVo) : Promise<any>
    {
        try
        {
            let recursos : any[] = this.gerarSQLConsultarListar(municipioVoFiltroPesquisa);
