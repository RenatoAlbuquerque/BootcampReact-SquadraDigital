...
class UFBe extends AbstractBe
{
    ...
    {
        if(ufVo == null || ufVo == undefined){throw new CampoObrigatorioError(acao, "o JSON ");}
        if(ufVo.sigla == undefined || ufVo.sigla == null || ufVo.sigla.trim() == ""){throw new CampoObrigatorioError(acao, "sigla");}
        if(ufVo.nome == undefined || ufVo.nome == null || ufVo.nome.trim() == ""){throw new CampoObrigatorioError(acao, "nome")}
        if(ufVo.status == undefined || ufVo.status == null || ufVo.status < 1){throw new CampoObrigatorioError(acao, "status")}
        if(alteracao && (ufVo.codigoUF == undefined || ufVo.codigoUF == null || ufVo.codigoUF < 1)){throw new CampoObrigatorioError(acao, "codigoUF")}
    }
}
